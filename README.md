...
**分层+领域聚合**架构,实现高内聚、低耦合:

- **app/** - 应用级入口,统一管理路由、插件、全局样式
- **domains/** - 按业务领域聚合,每个领域包含自己的组件、状态、服务
  - **editor** - 编辑器相关功能(编辑、历史、AI工具)
  - **library** - 图书与实体管理(书籍、章节、实体、草稿)
  - **settings** - 设置与配置(基础设置、AI配置、预设)
- **pages/** - 页面组件,组合各领域功能
- **shared/** - 跨领域共享模块(组件、工具、类型)
- **core/** - 核心基础设施(API封装)

**路径别名配置**:
- `@` → `src/` - 根目录
- `@app` → `src/app/` - 应用层
- `@domains` → `src/domains/` - 领域层
- `@pages` → `src/pages/` - 页面层
